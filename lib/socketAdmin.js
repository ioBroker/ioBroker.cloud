...
class SocketAdmin extends SocketCommon {
    ...
    start(server, socketClass, authOptions, socketOptions) {
        ...
        this.commands.start(thresholdEnabled =>
            this.onThresholdChanged(thresholdEnabled));
    }
    ...
    stateChange(id, state) {
        ...
        if (this.server && this.server.sockets) {
            ...
            sockets.forEach(socket =>
                this.commands.publish(socket, 'stateChange', id, state));
        }
    }
    ...
    fileChange(id, fileName, size) {
        if (this.server && this.server.sockets) {
            ...
            sockets.forEach(socket =>
                this.commands.publishFile(socket, id, fileName, size));
        }
    }
    ...
    objectChange(id, obj) {
        if (this.server && this.server.sockets) {
            ...
            sockets.forEach(socket =>
                this.commands.publish(socket, 'objectChange', id, obj));
        }
    }
    ...
    subscribeFile(id, pattern) {
        this.commands.subscribe(null, 'fileChange', id, pattern);
    }

    sendCommand(obj) {
        if (this.commands.sendCommand(obj) && this.server && this.server.sockets) {
            const sockets = this.server.sockets.sockets || this.server.sockets.connected;
    ...
    updateRatings(uuid, isAutoUpdate) {
        return this.commands.updateRatings(uuid, isAutoUpdate);
    }
}
