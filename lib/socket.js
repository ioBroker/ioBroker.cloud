...
function IOSocket(server, settings, adapter, lovelaceServer) {
    ...
    */
            ...
            that.commands.cloudDisconnect = function (socket, socketId, err) {
                ...
                if (socket._subSockets && socketId) {
                    if (socket._subSockets[socketId]) {
                        ...
                        that.adapter.log.warn('Received disconnection for unexisting socketId: ' + socketId)
                    }
                } else {
    ...
    function sendToLovelace(ioSocket, remoteSocketId, dataStr) {
        ...
        if (llSocket && llSocket.connected) {
            ...
            llSocket.socket = new WebSocket(lovelaceServer.replace(/^https/, 'wss').replace(/^http/, 'ws') + '/api/websocket');
            ...
            llSocket.socket.on('message', message => {
                llSocket.connected = true;
