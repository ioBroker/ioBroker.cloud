    ...
    */
    ...
    function initCommands() {
        that.commands.authenticate = function (socket, user, pass, callback) {
            ...
            if (socket._acl.user !== null) {
                typeof callback === 'function' && callback(socket._acl.user !== null, socket._secure);
            } else {
                that.adapter.log.debug((new Date()).toISOString() + ' Request authenticate [' + socket._acl.user + ']');
    ...
    function socketEvents(socket, address) {
        ...
        if (!socket.conn) {
                ...
                if (that.commands[command]) {
                    ...
                    switch (arguments.length) {
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            that.commands[command](socket, arguments[2]);
                            break;
                        case 3:
                            that.commands[command](socket, arguments[2], arguments[3]);
                            break;
                        case 4:
                            that.commands[command](socket, arguments[2], arguments[3], arguments[4]);
                            break;
                        case 5:
                            that.commands[command](socket, arguments[2], arguments[3], arguments[4], arguments[5]);
                            break;
                        case 6:
                            that.commands[command](socket, arguments[2], arguments[3], arguments[4], arguments[5], arguments[6]);
                            break;
                        case 7:
                            that.commands[command](socket, arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], arguments[7]);
                            break;
                        default:
                            that.commands[command](socket, arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], arguments[7], arguments[8]);
                            break;
                    }
                } else {
                    adapter.log.error('Received unknown command: ' + command);
            ...
            socket.on('cloudVersion', apiVersion => {
            ...
            socket.on('connect', () => {
                ...
                this.emit('apikey', that.settings.apikey, that.settings.version, that.settings.uuid, (err, instructions) => {
                    // instructions = {
