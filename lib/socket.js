    ...
    function initCommands() {
        ...
        if (that.settings.allowAdmin) {
            ...
            that.commands.decrypt = function (socket, encryptedText, callback) {
                if (that.secret) {
                    typeof callback === 'function' && callback(null, that.adapter.tools.decrypt(that.secret, encryptedText));
                } else {
            ...
            that.commands.encrypt = function (socket, plainText, callback) {
                if (that.secret) {
                    typeof callback === 'function' && callback(null, that.adapter.tools.encrypt(that.secret, plainText));
                } else {
                    adapter.getForeignObject('system.config', (err, obj) => {
                        if (obj && obj.native && obj.native.secret) {
                            that.secret = obj.native.secret;
