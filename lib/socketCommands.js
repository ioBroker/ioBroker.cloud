...
class SocketCommands {
        ...
        if (socket._acl.user !== 'system.user.admin') {
            ...
            if (SocketCommands.COMMANDS_PERMISSIONS[command]) {
                ...
                if (SocketCommands.COMMANDS_PERMISSIONS[command].type) {
                    if (
                        socket._acl[SocketCommands.COMMANDS_PERMISSIONS[command].type] &&
                        socket._acl[SocketCommands.COMMANDS_PERMISSIONS[command].type][
                            SocketCommands.COMMANDS_PERMISSIONS[command].operation
                            ]
                    ) {
                        return true;
                    } else {
                        this.adapter.log.warn(
                            `No permission for "${socket._acl.user}" to call ${command}. Need "${SocketCommands.COMMANDS_PERMISSIONS[command].type}"."${SocketCommands.COMMANDS_PERMISSIONS[command].operation}"`
                        );
                    }
                } else {
                    return true;
    ...
    subscribe(socket, type, pattern, patternFile) {
        ...
        if (this.subscribes[type][key] === undefined) {
            ...
            if (type === 'stateChange') {
                this.adapter.subscribeForeignStatesAsync(pattern, options)
                    .catch(e => this.adapter.log.error(`Cannot subscribe "${pattern}": ${e.message}`));
            } else if (type === 'objectChange') {
                this.adapter.subscribeForeignObjectsAsync(pattern, options)
                    .catch(e => this.adapter.log.error(`Cannot subscribe "${pattern}": ${e.message}`));
            } else if (type === 'log') {
                ...
                this.adapter.subscribeForeignFiles(pattern, patternFile, options)
                    .catch(e => this.adapter.log.error(`Cannot subscribe "${pattern}": ${e.message}`));
            }
        } else {
    ...
    async unsubscribe(socket, type, pattern, patternFile) {
        if (!pattern) {
            return this.adapter.log.warn('Empty pattern on subscribe!');
        }
        ...
        if (socket && typeof socket === 'object') {
                ...
                if (socket.subscribe[type][i].pattern === key) {
                    ...
                    if (this.subscribes[type][key] !== undefined) {
                        ...
                        if (this.subscribes[type][key] <= 0) {
                            if (type === 'stateChange') {
                                this.adapter.unsubscribeForeignStatesAsync(pattern, options)
                                    .catch(e => this.adapter.log.error(`Cannot unsubscribe "${pattern}": ${e.message}`));
                            } else if (type === 'objectChange') {
                                this.adapter.unsubscribeForeignObjectsAsync(pattern, options)
                                    .catch(e => this.adapter.log.error(`Cannot unsubscribe "${pattern}": ${e.message}`));
                            } else if (type === 'log') {
                                if (this.logEnabled && this.adapter.requireLog) {
                                    this.logEnabled = false;
                                ...
                                this.adapter.unsubscribeForeignFiles(pattern, patternFile, options)
                                    .catch(e => this.adapter.log.error(`Cannot unsubscribe "${pattern}": ${e.message}`));
                            }
            ...
            for (const pattern of Object.keys(this.subscribes[type])) {
                if (type === 'stateChange') {
                    ...
                    this.adapter.unsubscribeForeignFiles(id, fileName, options)
                        .catch(e => this.adapter.log.error(`Cannot unsubscribe "${pattern}": ${e.message}`));
                }
            }
    ...
    async subscribeSocket(socket, type) {
        if (!socket || !socket.subscribe) {
            return;
        }
        ...
        for (let i = 0; i < socket.subscribe[type].length; i++) {
            ...
            if (this.subscribes[type][pattern] === undefined) {
                ...
                if (type === 'stateChange') {
                    ...
                    const [id, fileName] = pattern.split('####');
                    this.adapter.subscribeForeignFiles(id, fileName, options)
                        .catch(e => this.adapter.log.error(`Cannot subscribe "${pattern}": ${e.message}`));
                }
            } else {
    ...
    async unsubscribeSocket(socket, type) {
        if (!socket || !socket.subscribe) {
            return;
        }
                                ...
                                if (data) {
                                    try {
                                        ...
                                        this.adapter.log.error(
                                            `[getBinaryState] Cannot convert data: ${error.toString()}`
                                        );
                                    }
                                }
