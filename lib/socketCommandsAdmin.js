...
class SocketCommandsAdmin extends SocketCommands {
    ...
    async updateRatings(uuid) {
        ...
        try {
            const response = await axios.get('https://rating.iobroker.net/rating?uuid=' + uuid, {timeout: 15000, validateStatus: status => status < 400})
            this.adapter._ratings = response.data;
            if (!this.adapter._ratings || typeof this.adapter._ratings !== 'object' || Array.isArray(this.adapter._ratings)) {
            ...
            if (this.adapter.name === 'admin') {
                ...
                this.adapter.ratingTimeout = setTimeout(() => {
                    ...
                    this.updateRatings(uuid, true)
                        .then(() => this.adapter.log.info('Adapter rating updated'));
                }, 24 * 3600000);
            }
    ...
    _sendToHost(host, command, message, callback) {
        ...
        //console.log(`Command: ${command} (${host})`);

        this.adapter.sendToHost(host, command, message, res => {
            //console.log(`Command: ${command} (${JSON.stringify(res)})`);
    ...
    __initCommandsObjects() {
        ...
        this.commands['getAllObjects'] = (socket, callback) => this._getAllObjects(socket, callback);

        // Identical to getAllObjects
        this.commands['getObjects'] = (socket, callback) => this._getAllObjects(socket, callback);
