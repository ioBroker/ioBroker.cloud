function AlexaSH2(adapter) {
    ...
    // collect all information for all ids
    function collectInformation(ids, controlCallback) {
        var allObjects = [];
        var objNumber = 0;
        var collectObjectInformationRec = function() {
            adapter.getForeignObject(ids[objNumber], function (err, obj) {
                allObjects.push(obj);
                objNumber ++;
                if (objNumber < ids.length) {
                    collectObjectInformationRec();
                }
                else {
                    controlCallback(allObjects);
                }
                return;
            });
        }
    ...
    function controlOnOff(allObjects, id, value, writeStates, callback) {
        // do not control numbers, when there is a switch in the collection
        var hasSwitch = false;
        var hasNumber = false;

        allObjects.forEach(function(otherObject) {
            if (otherObject.common.role.match(/^switch/)) {
                hasSwitch = true;
            }
            else if (otherObject.common.type === 'number') {
                hasNumber = true;
            }
        });
        ...
        adapter.getForeignObject(id, function (err, obj) {
            ...
            const byON = (
                ...
                typeof obj.common.smartName === 'object'
                ) ? obj.common.smartName.byON : null;
            const hasNonDefaultOnLevel = byON != null && byON !== '-';

            // if there's a switch in the configuration and this is not one, we're going to skip it
            if (hasSwitch && !obj.common.role.match(/^switch/)) {
                // except if we're turning it on AND it has an "on level" defined that is not "last stored value"
                if (!(value && hasNonDefaultOnLevel)) {
                    adapter.log.debug('skipping ' + id + ', cause no switch');
                    if (callback) callback();
                    return;
                }
            }
            ...
            adapter.log.debug('controlling ' + id + ', ' + obj.common.role + ', ' + value);
    ...
    function controlPercent(allObjects, id, value, writeStates, callback) {
        adapter.getForeignObject(id, function (err, obj) {
            if (!obj || obj.type !== 'state') {
                if (err) adapter.log.error('Cannot control non state: ' + id + ' ' + (obj ? obj.type : 'no object'));
                if (callback) callback();
            ...
            // do not control hue or saturation when percent is controlled, to prevent color or saturation change, when brightness is controlled
            var hasDimmer = false;
            var hasNumber = false;
            allObjects.forEach(function(otherObject) {
                if (otherObject.common.role.indexOf('.dimmer') !== -1) {
                    hasDimmer = true;
                }
                else if (otherObject.common.type === 'number') {
                    hasNumber = true;
            ...
            if (hasDimmer && obj.common.role.indexOf('.dimmer') === -1) {
                // there is a dimmer in the collection, so only control this
                adapter.log.debug('skipping ' + id + ', cause no dimmer');
                if (callback) callback();
                return;
            }

            if (hasNumber && obj.common.type !== 'number') {
                // there are numbers in the collection, so only control these
                adapter.log.debug('skipping ' + id + ', cause no number');
                if (callback) callback();
                return;
            }
            ...
            adapter.log.debug('setting percentage of ' + id + ' to ' + value);
        ...
        adapter.getForeignObject(idh, function (err, obj) {
            ...
            adapter.setForeignState(idh, color.hue, function (err) {
                ...
                adapter.getForeignObject(ids, function (err, obj) {
                    // prevent crash, when controlling hue
    ...
    this.process = function (request, isEnabled, callback) {
        ...
        switch (request.header.name) {
            ...
            case 'TurnOnRequest':
                ...
                var controlCallback = function(allObjects) {
                    for (i = 0; i < ids.length; i++) {
                        controlOnOff(allObjects, (idos && idos[i] !== undefined) ? idos[i] : ids[i], true, ids.length === 1, function () {
                            if (!--count) {
                ...
                collectInformation(ids, controlCallback);
            ...
            case 'TurnOffRequest':
                ...
                var controlCallback = function(allObjects) {
                    for (i = 0; i < ids.length; i++) {
                        controlOnOff(allObjects, (idos && idos[i] !== undefined) ? idos[i] : ids[i], false, ids.length === 1, function (err) {
                            if (!--count) {
            ...
            case 'SetPercentageRequest':
                ...
                var controlCallback = function(allObjects) {
                    // all information collected, now control the states
                    for (i = 0; i < ids.length; i++) {
                        controlPercent(allObjects, ids[i], request.payload.percentageState.value, ids.length === 1, function (err) {
                        if (!--count) {
