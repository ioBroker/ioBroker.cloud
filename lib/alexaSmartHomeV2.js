function AlexaSH2(adapter) {
                ...
                switch (operation) {
                    ...
                    case '%':
                        if (lang === 'de') {
                            ...
                            text = obj.friendlyName + ' <emphasis>set to</emphasis> ' + value + ' percent';
                        }
                        break;

                    case 'K':
                        if (lang === 'de') {
                            text = obj.friendlyName + ' wird auf ' + value + ' Kelvin <emphasis>gesetzt</emphasis>';
                        } else if (lang === 'ru') {
                            text = 'Состояние ' + obj.friendlyName + ' <emphasis>установлено</emphasis> на ' + value;
                        } else {
                            text = obj.friendlyName + ' <emphasis>set to</emphasis> ' + value + ' Kelvin';
                        }
                        break;
    ...
    function processState(states, id, room, func, alexaIds, groups, names, result) {
        try {
            ...
            const unit = states[id].common.unit;
            const role = states[id].common.role;
            ...
            if (states[id].common.write === false) {
                ...
                if (type === 'number') {
                    if (/^level\.color\./.test(role)) {
                        // test for color first, since color temperature may be given in Kelvin (K)
                        if (role === 'level.color.hue' || role === 'level.color.rgb') {
                            actions = ['setColor', /*'incrementColorTemperature', 'decrementColorTemperature', 'setColorTemperature', */'turnOn', 'turnOff'];
                        } else if (role === 'level.color.temperature') {
                            actions = ['setColorTemperature', /*'incrementColorTemperature', 'decrementColorTemperature'*/];
                        }
                    } else if (unit === 'C' || unit === 'C°' || unit === '°C' ||
                        unit === 'F' || unit === 'F°' || unit === '°F' ||
                        unit === 'K' || unit === 'K°' || unit === '°K') {
                        actions = ['setTargetTemperature', 'incrementTargetTemperature', 'decrementTargetTemperature', 'getTargetTemperature'];
                        type = '';
                    } 
                    // Fallback, in case none of the above matched:
                    if (actions == null) {
                        actions = ['setPercentage', 'incrementPercentage', 'decrementPercentage', 'turnOn', 'turnOff'];
                    }
                } else if (role === 'switch.lock') {
                    actions = ['setLockState', 'getLockState'];
                    type = '';
                } else if (role && role.match(/^button/)) {
                    actions = ['turnOn'];
                    type = '';
                } else {
    ...
    function controlColorTemperature(id, value, writeStates, callback) {
        adapter.getForeignObject(id, function (err, obj) {
            if (!obj || obj.type !== 'state') {
                if (err) adapter.log.error('Cannot control non state: ' + id + ' ' + (obj ? obj.type : 'no object'));
                if (callback) callback();
    ...
    this.process = function (request, isEnabled, callback) {
        ...
        switch (request.header.name) {
            ...
            case 'SetColorTemperatureRequest':
                adapter.log.debug('ALEXA SetColorTemperature: ' + request.payload.appliance.applianceId + ' ' + request.payload.colorTemperatureInKelvin + '%');
                for (i = 0; i < ids.length; i++) {
                    controlColorTemperature(ids[i], request.payload.colorTemperatureInKelvin, ids.length === 1, function (err) {
                        if (!--count) {
                            request.header.name = 'SetColorTemperature';
                ...
                writeResponse(request.payload.appliance.applianceId, 'K', request.payload.colorTemperatureInKelvin);
                break;
