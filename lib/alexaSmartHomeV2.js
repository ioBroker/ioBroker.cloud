        ...
        adapter.getForeignObject(id, function (err, obj) {
            if (!obj || obj.type !== 'state') {
                ...
                return;
						...
						adapter.log.debug('skipping test, hasSwitch: ' + hasSwitch + ', role: ' + obj.common.role);
            if (hasSwitch && !obj.common.role.match(/^switch/)) {
                // is not a switch, but configuration has a switch, so do nothing, only level passes through, but only in turn on state
								adapter.log.debug('skipping test 2: value: ' + value);
								if (!value || (value && !obj.common.role.match(/^level.dimmer/))) {
