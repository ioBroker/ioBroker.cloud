...
class CloudSshTunnel {
    ...
    open(id, host, port) {
        ...
        socket.on('data', (chunk) => this.emit('sshData', id, chunk.toString('base64')));
        socket.on('error', (e) => {
    ...
    isAllowed(host, port) {
        return this.rules.some(rule => CloudSshTunnel.hostMatches(rule.host, host) && CloudSshTunnel.portMatches(rule.ports, port));
    }
    /** Match a requested host against one rule's host: exact, `*` wildcard, CIDR, or range. */
    static hostMatches(pattern, host) {
...
exports.CloudSshTunnel = CloudSshTunnel;
//# sourceMappingURL=sshTunnel.js.map