...
class CloudAdapter extends adapter_core_1.Adapter {
    ...
    async onStateChange(id, state) {
        if (id.endsWith('remote.command')) {
            ...
            if (state?.val) {
                        ...
                        if (data.name === 'alive') {
                            ...
                            if (data.name === 'currentLocation') {
                                try {
                                    ...
                                    await this.setStateAsync(`${deviceId}.position`, `${json?.coords?.longitude};${json?.coords?.latitude}`, true);
                                }
                                catch {
                ...
                this.log.warn('Received an empty command: the value of the app did not arrive. A cloud that ' +
    ...
    static requestBodyToText(body) {
        ...
        const serializedBuffer = body;
        if (serializedBuffer.type === 'Buffer' && Array.isArray(serializedBuffer.data)) {
            return Buffer.from(serializedBuffer.data).toString('utf8');
        }
     ...
     */
    async processStateRequest(url, options, cb) {
                ...
                if (!obj) {
                    ...
                    if (state !== null && state !== undefined) {
                        ...
                        if (new URLSearchParams(query || '').has('json')) {
                            ...
                            cb(null, 200, headers, state.val === undefined
                                ? 'undefined'
                                : state.val === null
                                    ? 'null'
