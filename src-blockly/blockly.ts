/**
 * The ioBroker.cloud block for the Blockly editor of ioBroker.javascript.
 *
...
const iftttWords: Record<string, Record<string, string>> = {};
for (const [lang, texts] of Object.entries(LANGUAGES)) {
    for (const [word, text] of Object.entries(texts)) {
        if (text) {
            (iftttWords[word] ||= {})[lang] = text;
        }
    }
}
Object.assign(Blockly.Words, iftttWords);

// Links, not words, so they must not be given to the translator. German and Russian point at the
// documentation portal, every other language falls back to the English entry - which is what the
// hand-written table spelled out once per language.
...
function iftttToJavaScript(block: Block): string {
    ...
    // value1..3 are optional inputs and the event can be disconnected too. Emitting `value1: ,`
    // would break the user's whole script with a syntax error, so unconnected inputs are left out
    // entirely - which is what the adapter sees anyway, it forwards whatever keys the message carries.
